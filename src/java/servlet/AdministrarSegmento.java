...
public class AdministrarSegmento extends HttpServlet {
    ...
    private void agregar(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            salida.print("El Segmento ya existe");
        } catch (Exception e) {
            salida.print("Ha ocurrido una excepcion: " + e.toString());
        }
    }
    ...
    private void editar(HttpServletRequest request, HttpServletResponse response) throws Exception {
        PrintWriter salida = response.getWriter();
        try {
            int linea = Integer.parseInt(request.getParameter("id_linea_ed"));
            boolean recta = Boolean.parseBoolean(request.getParameter("tipo_segmento_ed"));
            double pkInicio = Double.parseDouble(request.getParameter("pk_inicio_ed"));
            double pkFinal = Double.parseDouble(request.getParameter("pk_final_ed"));
            double radioCurvatura = Double.parseDouble(request.getParameter("radio_ed"));
            double gradiente = Double.parseDouble(request.getParameter("gradiente_ed"));
            double rampaAscendente;
            double rampaDescendente;
            if (radioCurvatura > 0) {
                rampaAscendente = gradiente + (600 / radioCurvatura);
            ...
            boolean tunel = Boolean.parseBoolean(request.getParameter("tunel_ed"));
            double velocidadMaxAscendente = Double.parseDouble(request.getParameter("velocidad_max_ascendente_ed"));
            double velocidadMaxDescendente = Double.parseDouble(request.getParameter("velocidad_max_descendente_ed"));
            ...
            sjc.edit(seg);
            salida.print("Segmento con Progresiva Inicial " + seg.getSegmentoPK().getIdPkInicial() + " ha sido editado satisfactoriamente");
        } catch (PreexistingEntityException e) {
            salida.print("El Segmento ya existe");
        } catch (Exception e) {
    ...
    private void eliminar(HttpServletRequest request, HttpServletResponse response) throws NonexistentEntityException, ServletException, IOException {
        ...
        try {
            ...
            sjc.destroy(spk);
            salida.print("Segmento eliminado satisfactoriamente");
